...
const generateReport = async () => {
  ...
  try {
    // 1. Set up listeners
    const unlistenChunk = await listen<string>('report-stream-chunk', (event) => {
      generateStore.generatedContent += event.payload
    })
    const unlistenDone = await listen<string>('report-stream-done', async (event) => {
      generateStore.generatedContent = event.payload
    ...
    // 2. Start generation
    const prompt = prompts.value.find(p => p.id === generateStore.selectedPromptId)
...
<template>
  <div class="h-full flex overflow-hidden bg-bg-base">
    <!-- Left Panel: Config -->
    <div class="w-80 flex-shrink-0 border-r border-border-base bg-bg-container flex flex-col shadow-sm">
      <div class="p-5 border-b border-border-base">
        ...
        <div class="space-y-6">
          <!-- Repos -->
          <div>
            <label class="block text-[11px] font-bold text-text-placeholder uppercase tracking-wider mb-2.5">选择仓库</label>
            <div class="space-y-1 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
              <div 
                v-for="repo in repoStore.repositories" 
                :key="repo.id"
                @click="selectedRepoIds.includes(repo.id) ? selectedRepoIds = selectedRepoIds.filter(id => id !== repo.id) : selectedRepoIds.push(repo.id)"
                class="flex items-center gap-2.5 px-2 py-1.5 rounded-md cursor-pointer transition-colors"
                :class="[selectedRepoIds.includes(repo.id) ? 'bg-primary/10 text-primary' : 'hover:bg-black/5 dark:hover:bg-white/5 text-text-secondary']"
              >
                <div class="w-3.5 h-3.5 rounded border border-border-base flex items-center justify-center transition-colors" :class="{'bg-primary border-primary': selectedRepoIds.includes(repo.id)}">
                  <el-icon v-if="selectedRepoIds.includes(repo.id)" :size="10" color="white"><Select /></el-icon>
                </div>
                <span class="text-[13px] truncate font-medium">{{ repo.name }}</span>
              </div>
            </div>
          </div>
          ...
          <!-- Date Range -->
          <div>
            <label class="block text-[11px] font-bold text-text-placeholder uppercase tracking-wider mb-2">日期范围</label>
            <el-date-picker
              v-model="dateRange"
              type="daterange"
              range-separator="-"
          ...
          <!-- Prompt Template -->
          <div>
            <label class="block text-[11px] font-bold text-text-placeholder uppercase tracking-wider mb-2">生成模板</label>
            <el-select v-model="generateStore.selectedPromptId" placeholder="选择模板" size="small" class="!w-full">
              <el-option
          ...
          >
            <el-icon v-if="commitsLoading" class="animate-spin mr-1"><Loading /></el-icon>
      ...
      <div class="flex-1 overflow-hidden flex flex-col">
        <div class="px-5 py-3 flex justify-between items-center bg-bg-base/50 border-b border-border-base">
          <span class="text-[11px] font-bold text-text-placeholder uppercase tracking-wider">提交预览 ({{ generateStore.commits.length }})</span>
        </div>
        <div class="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
          <div v-if="generateStore.commits.length === 0" class="text-center py-12 opacity-20">
          ...
          >
            <div class="flex justify-between items-center mb-1">
              <span class="text-[10px] font-mono text-primary font-bold">{{ commit.shortHash }}</span>
              <span class="text-[10px] text-text-placeholder">{{ commit.repoName }}</span>
            </div>
            <p class="text-[12px] text-text-secondary line-clamp-2 leading-snug">{{ commit.message }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="flex-1 flex flex-col bg-bg-base border-l border-border-base relative">
      ...
      <header class="min-h-[56px] flex-shrink-0 flex items-center justify-between px-8 border-b border-border-base bg-bg-container relative z-10 flex-nowrap">
        ...
        <div class="flex gap-2 shrink-0">
          ...
          >
            <el-icon v-if="generating" class="animate-spin mr-1"><Loading /></el-icon>
      ...
      <!-- Content Area -->
      <div class="flex-1 overflow-y-auto p-12 relative z-10 custom-scrollbar flex justify-center">
        <transition name="fade-up" mode="out-in">
          <div v-if="generating" class="h-full flex flex-col items-center justify-center text-center">
            <el-icon class="animate-spin text-primary mb-4" :size="48"><Loading /></el-icon>
            <h3 class="text-xl font-bold text-text-title mb-1.5">正在分析数据...</h3>
            <p class="text-[13px] text-text-secondary">AI 正在根据您的提交记录编撰日报</p>
          </div>

          <div v-else-if="generateStore.generatedContent" class="w-full max-w-4xl">
          ...
          <div v-else class="h-full flex flex-col items-center justify-center text-center opacity-40 select-none">
            <div class="w-24 h-24 border border-dashed border-border-base rounded-2xl flex items-center justify-center mb-5">
              <el-icon :size="32" class="text-text-placeholder"><MagicStick /></el-icon>
            </div>
            <h3 class="text-xl font-bold text-text-title">待生成</h3>
            <p class="text-[13px] mt-2 max-w-xs leading-relaxed text-text-secondary">
